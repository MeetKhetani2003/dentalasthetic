...
import { Transformation } from "@/db/schema";
...
export async function POST(req: Request) {
  try {
    ...
    const newDoc = await Transformation.create(body);
...
export async function PUT(req: Request) {
  try {
    ...
    const updated = await Transformation.findByIdAndUpdate(_id, rest, { new: true });
